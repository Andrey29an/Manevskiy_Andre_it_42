...
замовчуванням. Ця різниця стане принциповою знову в Лекції 15, коли
...
Лекції 5 було сказано, що заповнення пропуску середнім штучно зменшує
...
рецептом із Лекції 5 його заповнили середнім (20.0):
...
Міжквартильний розмах — це вже знайомий з Лекції 5 інструмент, тільки
...
у Лекції 5: якщо сам розмах, за яким шукають викиди, було б легко
...
один пік біля 31500, і саме гістограма (Лекція 6) — перший інструмент,
...
цій лекції та Лекції 17), графіки до наукових статей і звітів
...
області це стає незручним. У Лекції 8 сітка `fig, axes =
...
- `ax.plot()` — лінія (типово для послідовних даних, наприклад
  часового ряду);
- `ax.hist()` — гістограма частот однієї змінної (детальніше — Лекція 6);
...
- **"вуса"** — відрізки, що тягнуться від ящика до найдальшого
  значення, яке ще не вважається викидом за критерієм із Лекції 5:
  межа `Q1 - 1.5 * IQR` знизу і `Q3 + 1.5 * IQR` зверху;
- **окремі точки за межами вусів** — значення поза цими межами, тобто
  ті самі викиди, які Лекція 5 виявляла формулою; boxplot просто малює
...
верхнім вусом — так виглядає той-таки IQR-критерій із Лекції 5, лише
...
- Заповнення пропуску середнім (Лекція 5) механічно зменшує std: додане
...
- Boxplot — це наочне зображення медіани, IQR і меж викидів за тим самим
  критерієм `1.5 * IQR`, що вже застосовувався в Лекції 5.
...
Семенович К.О., Лавренюк М.В., Пришляк О.О. *Основи візуалізації наукових даних: Конспект лекцій з курсу «Візуалізація даних».* — Київ: Київський національний університет імені Тараса Шевченка (мех.-мат. ф-т, каф. комп'ютерних методів механіки і процесів керування), 2024 ([відкритий доступ](https://mechmat.knu.ua/wp-content/uploads/2024/05/elektronnyj-konspekt-lektsij-z-kursu-vizualizatsiia-danykh_osnovy-vizualizatsii-naukovykh-danykh.pdf)). Розділи 2-4: основи Matplotlib, налаштування графіків, візуалізація похибок і розподілів — безпосередньо міри розсіювання та їх графічне представлення.

Шарадкін Д.М., Субач І.Ю., Микитюк А.В. *Інструментальні засоби Python для моделювання та системного аналізу часових рядів при вирішенні задач кіберзахисту інформаційно-комунікаційних систем.* — Київ: ІСЗЗІ КПІ ім. Ігоря Сікорського, 2023 ([відкритий доступ](https://ela.kpi.ua/bitstream/123456789/52426/1/Instrumentalni_zasoby_Python_2023.pdf)). Розділ 5 «Засоби візуалізації в аналізі даних» і розділ 6 «Гістограми: засоби побудови та аналізу».
