...
*Лекція 12 показала, що вибіркове середнє x̄ коливається навколо
істинного μ зі стандартною похибкою σ/√n, а Лекція 13 перетворила це
...
інтервалу в Лекції 13: висновок стосується процедури й даних, а не
...
Лекції 10 показує різницю наочно: "ймовірність позитivного тесту за
...
менша стандартна похибка σ/√n з Лекції 12 → легше відрізнити
...
## Зв'язок із довірчими інтервалами (Лекція 13): та сама відповідь, два способи запитати
...
стандартна похибка se = 15/√36 = 2.5 г (Лекція 12). 95%-й довірчий
інтервал (Лекція 13): 496 ± 1.96·2.5 = [491.1; 500.9] — і μ0 = 500 г
...
n = 36
se = s / np.sqrt(n)     # стандартна похибка, Лекція 12: se = s/√n
...
предмет Лекції 15, де t-тест — пряме застосування всієй логіки,
...
Лекції 16 для критерію хі-квадрат — там статистика рахується геть
...
- Двобічний тест на рівні α і (1 − α)-довірчий інтервал (Лекція 13)
...
Каніовська І.Ю., Стусь О.В. *Математична статистика: Збірник задач.* — Київ: КПІ ім. Ігоря Сікорського, 2019 ([відкритий доступ](https://ela.kpi.ua/bitstream/123456789/27540/1/Matemat_statystyka.pdf)). Гл.5 «Перевірка гіпотез про параметри розподілу» (прості та складні гіпотези) — профільний матеріал з p-value/статистичних гіпотез.

Єременко В.С., Куц Ю.В., Мокійчук В.М., Самойліченко О.В. *Статистичний аналіз даних вимірювань.* — Київ: Національний авіаційний університет (НАУ), 2013 ([відкритий доступ](https://ela.kpi.ua/bitstream/123456789/44902/1/statistical_analysis.pdf)). Гл.4 «Перевірка гіпотез про статистичні характеристики» (середні, дисперсії, однорідність, систематичні похибки).
