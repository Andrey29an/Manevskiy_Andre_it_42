...
*Лекція 14 дала загальну логіку перевірки статистичних гіпотез: H0, H1,
...
логіку Лекції 14 до середніх, спираючись на t-розподіл із Лекції 13.
Заодно ця лекція повертається до питання, яке Лекція 7 свідомо залишила
...
**Механіка.** Гіпотези за схемою Лекції 14:
...
вибіркове стандартне відхилення (`ddof=1`, Лекція 7), `n` — розмір
...
похибка середнього (детально виведена в Лекції 12), яка масштабує цю
різницю до одиниць "скільки стандартних похибок відхилення". Число
ступенів свободи тут `n - 1` — той самий `n - 1`, що й у поправці
Бесселя для дисперсії (Лекція 7): один ступінь свободи "витрачається" на
те, що `s` вже обчислено відносно самого `x̄`, а не відносно справжнього
невідомого `μ`.

**Чому t, а не z.** У Лекції 13 різниця між z- і t-розподілом уже
...
точно та сама, що в Лекції 14: якщо `p_value < α` (типово 0.05),
...
У Лекції 7 було зазначено: різниця між `ddof=0` і `ddof=1` "стане
принциповою знову в Лекції 15, коли йтиметься про порівняння дисперсій
...
p-value й тим самим апаратом Лекції 14.
...
навіть трохи скошений або має важкі хвости (Лекція 7), F-критерій
...
Це замикає коло, відкрите в Лекції 7: там `ddof=0` проти `ddof=1` — це
...
однобічних/двобічних альтернатив, що й у Лекції 14.
...
- Одновибірковий t-критерій (`ttest_1samp`) перевіряє, чи середнє однієї
  ...
  безпосередній застосунок логіки H0/p-value Лекції 14 з t-розподілом
  Лекції 13.
...
- Критерій Лівена (`scipy.stats.levene`) формально перевіряє рівність
  ...
  Welch-варіантом t-критерію, замикаючи питання, відкрите в Лекції 7.
...
Вишневецька В.П. *Основи математичної статистики з елементами хмаро орієнтованих технологій.* — Київ: Видавництво НПУ імені М.П. Драгоманова, 2019 ([відкритий доступ](https://files.znu.edu.ua/files/Bibliobooks/Inshi80/0060023.pdf)). Розділ «Параметричні критерії»: статистичні критерії перевірки гіпотез (у порівнянні з непараметричним критерієм Вілкоксона).

Єременко В.С., Куц Ю.В., Мокійчук В.М., Самойліченко О.В. *Статистичний аналіз даних вимірювань.* — Київ: Національний авіаційний університет (НАУ), 2013 ([відкритий доступ](https://ela.kpi.ua/bitstream/123456789/44902/1/statistical_analysis.pdf)). Гл.4: перевірка гіпотез про середні й дисперсії (t-критерій, F-критерій) для даних вимірювань.
