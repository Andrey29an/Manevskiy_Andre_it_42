...
*Лекція 16 навчила перевіряти зв'язок між двома **категоріальними**
...
і силу. Лекція 7 вже намалювала діаграму розсіювання годин підготовки
...
працював у дисперсії (Лекція 7) і статистиці хі-квадрат (Лекція 16),
...
Цей самий код і цей самий набір уже з'являлися в Лекції 7 як приклад
...
потрібна **коваріація** — пряме узагальнення дисперсії (Лекція 7) на
...
Це той самий прийом нормування, що коефіцієнт варіації (Лекція 7)
...
`result.pvalue` — це p-value того самого сорту, що в Лекції 14: H0 тут
...
сумісних із цими даними — та ж логіка довірчого інтервалу з Лекції 13,
...
добутків відхилень — точно як дисперсія (Лекція 7), вона підсилює
...
наявність точок, що явно випадають (за критерієм IQR з Лекції 5).
...
значеннями означає щось стабільне (Лекція 4: інтервальна чи
...
**Важливе застереження, що прямо веде до Лекції 18–19:** матриця
...
скасовують один одного (та сама причина, що в дисперсії, Лекція 7, і в
хі-квадраті, Лекція 16, — потрібно прибрати знаки). Модуль теж прибирає
...
логікою Лекції 14; мале значення означає, що нахил статистично значуще
...
одним предиктором; щойно предикторів стає кілька — тема Лекції 18 —
...
- `pandas.DataFrame.corr()` рахує всі попарні кореляції одразу, але
  ...
  предикторів вона не вміє (це задача множинної регресії, Лекція 18).
...
Множинна регресія (Лекція 18) — той самий принцип МНК і той самий
...
Єременко В.С., Куц Ю.В., Мокійчук В.М., Самойліченко О.В. *Статистичний аналіз даних вимірювань.* — Київ: Національний авіаційний університет (НАУ), 2013 ([відкритий доступ](https://ela.kpi.ua/bitstream/123456789/44902/1/statistical_analysis.pdf)). Гл.5 «Кореляційний аналіз»: коефіцієнт кореляції Пірсона, рангові кореляції Спірмена й Кендалла, часткова кореляція.

Данілов В.Я. *Статистична обробка даних.* — Київ: Київський національний університет імені Тараса Шевченка (мех.-мат. ф-т, каф. загальної математики), 2019 ([відкритий доступ](https://www.mechmat.univ.kiev.ua/wp-content/uploads/2020/03/statustushna-obrobka-danilov-2019.pdf)). Матеріал про двовимірні статистичні розподіли та кореляційний і регресійний аналіз даних.
