...
*Модуль 4 відкрився методом Монте-Карло (Лекція 21) — способом оцінити
...
досі: вибірка зросту студентів (Лекція 6), пари "години підготовки —
бал" (Лекція 17), навіть послідовність кидків кубика в симуляції
Монте-Карло — усе це **i.i.d.**-дані (Лекції 12–16): незалежні,
...
перемішування: для будь-якої статистики з Лекцій 6–16 (x̄, s²,
...
ЦГТ (Лекція 12) і всі критерії Лекцій 13–16 — застосовувати ці критерії
...
21 генерувались синтетичні набори з наперед відомими "правильними"
...
само, як діаграма розсіювання (Лекція 17) підказувала форму зв'язку
...
**Застереження з Лекції 8.** Правило "вісь Y стовпчикової діаграми
...
= Var(X)/n` з Лекції 12, лише застосована до ковзного, а не одного
...
у синтетичних наборах Лекції 21: якщо декомпозиція не відновлює
...
fit = linregress(x, trend_clean.values)              # той самий linregress, що в Лекції 17
...
Це пряме продовження Лекції 17: `linregress` тут підбирає пряму лінію
...
Лекція 17 сформулювала для простої регресії: рівняння `y = b0 + b1·x`,
...
**Застосування критеріїв Лекцій 13–16 до часового ряду без урахування
залежності між сусідніми точками.** Довірчі інтервали й перевірка
гіпотез побудовані на припущенні незалежних спостережень (Лекція 12);
...
- Часовий ряд — послідовність вимірювань однієї змінної в часі, де
  **порядок** несе інформацію, а сусідні спостереження зазвичай
  залежні; це принципова відмінність від i.i.d.-вибірок Лекцій 12–16,
...
- Лінійний графік ряду — перший крок аналізу "на око", з окремим
  застереженням Лекції 8: обрізана вісь Y для лінійного графіка часу
...
- `statsmodels.tsa.seasonal.seasonal_decompose` формалізує розділення
  ...
  не довільні числа — той самий принцип перевірки, що й у Лекції 21.
- Прогноз на кілька кроків — оглядово: продовження тренду через
  `linregress` (Лекція 17) плюс типовий сезонний ефект, або наївні
...
- Головна пастка — плутати "модель добре описує історичні дані" з
  ...
  екстраполяції, що й у простій регресії (Лекція 17), тільки гостріше,
  бо межа спостереженого діапазону в часовому ряді щоразу рухається
  вперед.

Наступна лекція (Лекція 23) переходить до зовсім іншого класу задач —
...
мережі для довших залежностей) — але це вже інструментарій Лекції 23,
...
Бідюк П.І., Романенко В.Д., Тимощук О.Л. *Аналіз часових рядів: навчальний посібник.* — Київ: НТУУ «КПІ», 2010 ([відкритий доступ](https://ela.kpi.ua/items/256da52e-13da-4d07-8f25-9d9505496a2c)). Профільний посібник з аналізу часових рядів: стаціонарність, ARMA-моделі, тренди, коінтеграція, оцінювання параметрів.

Єріна А.М., Мазуренко О.К. *Статистичний аналіз часових рядів.* — Київ: Київський національний університет імені Тараса Шевченка, 2022 ([відкритий доступ](https://idss.org.ua/arhiv/Yerina_Mazurenko_colour.pdf)). Профільний посібник: компонентний аналіз (тренд/сезонність/циклічність), адаптивні методи прогнозування (експоненційне згладжування, ARIMA).
