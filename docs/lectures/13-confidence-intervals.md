...
*Лекція 12 показала, що вибіркове середнє x̄ саме по собі — випадкова
...
висновку, на якому будуватиметься й перевірка гіпотез у Лекції 14.*
...
(Лекції 6–7, Практика 5–6) — і в цьому й проблема точкової оцінки: саме
...
саме σ/√n (або s/√n) з Лекції 12: чим вона менша, тим вужчий і
інформативніший інтервал при тому самому рівні довіри.

**Чому це працює саме так.** ЦГТ (Лекція 12) гарантує, що розподіл x̄
навколо μ наближається до нормального з розкидом σ/√n. А для будь-якого
нормального розподілу відомо (правило 68-95-99.7, Лекція 11), яка частка
...
а s — вибіркове стандартне відхилення (Лекція 7), яке тут відіграє роль
оцінки невідомого σ.
...
se = s / np.sqrt(n)                    # стандартна похибка (Лекція 12)
...
Це прямо той самий тип міркування, що симуляція з Лекції 12: там
...
біноміального розподілу `n·p·(1-p)` у Лекції 11 — тут та сама дисперсія
...
курс уже стикався в Лекції 11 при зв'язку біноміального розподілу з
...
логіка перевірки статистичних гіпотез у Лекції 14, де довірчий інтервал
...
той самий факт з Лекції 12: щоб зменшити ширину інтервалу вдвічі,
...
- Довірчий інтервал і перевірка статистичної гіпотези (Лекція 14) —
...
Каніовська І.Ю., Стусь О.В. *Математична статистика: Збірник задач.* — Київ: КПІ ім. Ігоря Сікорського, 2019 ([відкритий доступ](https://ela.kpi.ua/bitstream/123456789/27540/1/Matemat_statystyka.pdf)). Гл.3 «Побудова довірчих інтервалів для параметрів розподілу» — профільний розділ саме про довірчі інтервали.

Вишневецька В.П. *Основи математичної статистики з елементами хмаро орієнтованих технологій.* — Київ: Видавництво НПУ імені М.П. Драгоманова, 2019 ([відкритий доступ](https://files.znu.edu.ua/files/Bibliobooks/Inshi80/0060023.pdf)). Розділ «Вибіркові методи»: похибка вибірки, довірчі інтервали, критерії надійності оцінювання параметрів генеральної сукупності.
