...
стовпець із Лекції 3 (`df["новий"] = ...` векторизованою формулою або
...
регресії (Лекція 18) і класифікації математичних моделей (Лекція 19),
...
"молодих" і "старших" клієнтів через `groupby` (Лекція 8) чи `crosstab`,
...
(модуль 3, Лекції 18–19) обидва чутливі до масштабу вхідних змінних саме
таким чином.
...
викид (Лекція 5) стискає всі інші значення в вузьку смужку біля 0 чи 1,
...
середнього до викидів, що й у Лекції 6), лише дещо м'якше, ніж на min-max.
...
тож категоріальну змінну (Лекція 4) потрібно перетворити в числовий вигляд.
...
**Чому саме так, а не навпаки — пряме продовження Лекції 4.** Порядкове
...
жодного змісту для назви міста. Це той самий принцип, що в Лекції 4
...
- **Похідна змінна** будується тією самою механікою обчислюваних стовпців
  із Лекції 3, але свідомо: вона часто несе більше сенсу для аналізу чи
...
- **Індикаторне (one-hot) кодування** — для номінальних змінних, **порядкове
  ...
  Лекції 4 про шкали вимірювання).
...
Сергеєв-Горчинський О.О., Іщенко Г.В. *Інтелектуальний аналіз даних: комп'ютерний практикум.* — Київ: КПІ ім. Ігоря Сікорського, 2018 ([відкритий доступ](https://ela.kpi.ua/bitstream/123456789/24971/1/Komp_prakt.pdf)). Немає окремого україномовного підручника з feature engineering; використано матеріал про типи атрибутів (категоріальні, порядкові, інтервальні, числові) та якість даних (ARFF-формат) як найближчий за змістом.

Олещенко Л.М. *Машинне навчання: комп'ютерний практикум.* — Київ: КПІ ім. Ігоря Сікорського, 2022 ([відкритий доступ](https://ela.kpi.ua/bitstream/123456789/48731/1/Mashynne_navchannia_praktykum)). Практикуми з кластеризації й дерев рішень ілюструють практичне конструювання/відбір ознак (напр., підготовка змінних для k-means, для класифікації виживання пасажирів).
