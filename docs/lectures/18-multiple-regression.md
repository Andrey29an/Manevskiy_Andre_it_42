...
*Лекція 17 ввела просту лінійну регресію: одна пояснювальна змінна `x`,
...
регресії з Лекції 17 (`y = b0 + b1·x + ε`) — просто з одним доданком
...
відхилень), а не виводити її з нуля через лінійну алгебру. У Лекції 17
...
Лекції 17 у таких задачах — не помилка, а свідоме спрощення: вона
...
**Що таке statsmodels.** На відміну від SciPy (Лекція 11), яка дає лише
...
(Лекція 22), дисперсійний аналіз (ANOVA) та інші класичні методи, кожен
...
13-16 цього курсу (довірчі інтервали, перевірка гіпотез) — на відміну
від `sklearn`, що з'явиться в Лекції 23 і орієнтований на точність
...
- `statsmodels.OLS` створена для задач **статистичного висновку**:
  ...
  в Лекціях 13–16 (довірчі інтервали, перевірка гіпотез через p-value).
...
кожен коефіцієнт (Лекція 23 і машинне навчання), `sklearn` стане
...
перевірки гіпотез із Лекції 14–15 це означає, що для кожного з них є
...
R² вимірює частку дисперсії `y`, пояснену моделлю (Лекція 17). МНК
...
Логіка діагностики залишків із простої регресії (Лекція 17) переноситься
...
що в Лекції 16 для перевірки нормальності вибірки — тільки застосований
...
Логіка інтерпретації буквально та сама, що в Лекції 16: `p_value ≥
...
кривизна чи експоненційна форма на діаграмі розсіювання з Лекції 17),
...
- Множинна лінійна регресія `y = b0 + b1·x1 + ... + bk·xk + ε` —
  пряме узагальнення простої регресії з Лекції 17 на кілька
...
- Діагностика залишків множинної регресії — та сама логіка, що для
  ...
  QQ-plot і критерій Шапіро-Вілка з Лекції 16 (нормальність).
...
Єременко В.С., Куц Ю.В., Мокійчук В.М., Самойліченко О.В. *Статистичний аналіз даних вимірювань.* — Київ: Національний авіаційний університет (НАУ), 2013 ([відкритий доступ](https://ela.kpi.ua/bitstream/123456789/44902/1/statistical_analysis.pdf)). Гл.6 «Регресійний аналіз»: лінійна й нелінійна регресія, метод найменших квадратів, усунення тренду.

Данілов В.Я. *Статистична обробка даних.* — Київ: Київський національний університет імені Тараса Шевченка (мех.-мат. ф-т, каф. загальної математики), 2019 ([відкритий доступ](https://www.mechmat.univ.kiev.ua/wp-content/uploads/2020/03/statustushna-obrobka-danilov-2019.pdf)). Розділ 2 (гл.4-6) «Статистичний аналіз даних з пакетом STATISTICA»: моделі лінійної регресії на практичних прикладах.
