...
*Лекції 17–18 будували конкретні статистичні моделі — кореляційну
...
розглядаються в Лекції 20; імітаційні моделі — у Лекції 21 через метод
Монте-Карло; моделі часових рядів (Лекція 22) — це конкретний приклад
динамічної моделі в термінах, введених тут. А регресія з Лекцій 17–18 —
приклад статистичної моделі, чия адекватність перевіряється саме тими
інструментами, що вже введені в Лекції 16 (критерій хі-квадрат, тест на
нормальність).*
...
інвестиційного портфеля при обмеженому ризику. **Лекція 20** детально
...
21** розглядає це детально: генерацію випадкових потоків і метод
...
розглядає **Лекція 22** (аналіз часових рядів).
...
| **Стохастична** | Регресія із випадковим залишком (Лекції 17–18): та сама температура завжди дає той самий прогноз, але фактичне значення коливається навколо нього | Імітаційна модель черги в банку: довжина черги в момент `t` залежить від випадкових подій до моменту `t` |
...
**Прямий місток до Лекції 16.** Формальні критерії узгодженості з
Лекції 16 — це саме інструменти перевірки адекватності
...
регресії (Лекції 17–18) — ще один кількісний показник адекватності:
...
- Адекватність оцінюється порівнянням передбачень з реальними даними,
  ...
  критерії узгодженості з Лекції 16 і R² з Лекцій 17–18 — конкретні
...
побудови, — тема **Лекції 20**: як формально ставити задачу пошуку
...
Данілов В.Я. *Статистична обробка даних.* — Київ: Київський національний університет імені Тараса Шевченка (мех.-мат. ф-т, каф. загальної математики), 2019 ([відкритий доступ](https://www.mechmat.univ.kiev.ua/wp-content/uploads/2020/03/statustushna-obrobka-danilov-2019.pdf)). Розділ про лінійні регресійні моделі в STATISTICA — оцінювання якості/адекватності побудованої моделі на реальних даних.

Сергеєв-Горчинський О.О., Іщенко Г.В. *Інтелектуальний аналіз даних: комп'ютерний практикум.* — Київ: КПІ ім. Ігоря Сікорського, 2018 ([відкритий доступ](https://ela.kpi.ua/bitstream/123456789/24971/1/Komp_prakt.pdf)). Методологія CRISP-DM (побудова, тестування, впровадження моделі) — загальні принципи оцінки адекватності та класифікації типів моделей аналізу даних.
