...
в роботі чи навчанні). Лекції 17–18 будували модель того самого
...
під час навчання не бачила". Лекція 18 вже назвала цю пізнішу мету
...
регресія — Лекції 17-18), але тепер застосований до нової мети —
...
**Чому мета інша, ніж у Лекцій 17–18, навіть коли формула та сама.**
Статистична модель з Лекції 18 оцінюється тим, наскільки добре вона
...
зобов'язана бути інтерпретованою так, як наполягала Лекція 18 щодо
...
**Приклад навчання з учителем.** Дані про 22 квартири з Лекції 18, де
...
*(Термін "регресія" тут — те саме слово, що в Лекціях 17–18: "регресія"
...
центру — саме ця задача з Лекції 18 послужить наскрізним прикладом
далі.
...
- **навчальна вибірка** (training set) — на ній модель підбирає
  ...
  величину, що в Лекції 18 підбирав МНК);
...
# ті самі 22 квартири, що в Лекції 18
...
у Лекції 21: поділ включає випадкове перемішування рядків перед
...
Лекція 18 вже згадувала доданок `x²` як спосіб надати лінійній моделі
...
МНК (Лекції 17–18), — тож великі помилки штрафуються сильніше, ніж
...
знайомий з Лекцій 17–18, де він показував, яку частку дисперсії `y`
...
18 (`LinearRegression` сама додає вільний член, тож `sm.add_constant()`
тут не потрібен). `R2_train = 0.961` — модель добре узгоджена з 17
квартирами, на яких її навчали, порівнюване з `R² = 0.951` для повної
моделі з Лекції 18 (трохи інше число — очікувано, це вже інша, менша
...
принцип, що в Лекції 18 для adjusted R² при доданні предикторів:
...
- Машинне навчання з учителем шукає функцію `f`, що передбачає цільову
  ...
  на відміну від статистичної регресії Лекцій 17–18, метою якої було
...
- Accuracy оманлива при незбалансованих класах; precision, recall і
  ...
  17–18, тепер у ролі показника узагальнення, а не пояснення наявних
  даних.
...
Олещенко Л.М. *Машинне навчання: комп'ютерний практикум.* — Київ: КПІ ім. Ігоря Сікорського, 2022 ([відкритий доступ](https://ela.kpi.ua/bitstream/123456789/48731/1/Mashynne_navchannia_praktykum)). Профільний комп'ютерний практикум з машинного навчання: кластеризація k-means, дерева рішень, згорткові нейронні мережі.

Сергеєв-Горчинський О.О., Іщенко Г.В. *Інтелектуальний аналіз даних: комп'ютерний практикум.* — Київ: КПІ ім. Ігоря Сікорського, 2018 ([відкритий доступ](https://ela.kpi.ua/bitstream/123456789/24971/1/Komp_prakt.pdf)). Профільний практикум з інтелектуального аналізу даних: навчання з учителем/без учителя, класифікація, кластеризація, асоціативні правила, CRISP-DM.
