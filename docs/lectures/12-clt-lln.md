...
*Лекція 10 закінчилась відкритим питанням, яке свідомо залишили без
відповіді: вибіркове середнє x̄, яке рахували ще в Лекції 6, явно "має
стосунок" до математичного сподівання E[X] з Лекції 10 — але яке саме?
...
буквально побудовані довірчі інтервали (Лекція 13) і перевірка гіпотез
(Лекція 14): жодної нової магії там не буде, лише механічне застосування
...
кодом — візьмемо експоненційний розподіл (Лекція 11) з параметром
...
"довіра до x̄", яку Лекція 10 залишила без обґрунтування — тепер вона
...
експоненційний розподіл із Лекції 11) чи взагалі рівномірна — розподіл
x̄ₙ при достатньо великому n наближається до **нормального** розподілу
(Лекція 11) із параметрами:
...
оцінка E[X]; s² (Лекція 7) — оцінка Var(X). Не кожне правило, що
...
n−1 (`ddof=1`), а не на n (Лекція 7) — це саме питання незміщеності:
...
## Де це реально корисно: фундамент для Лекцій 13–14
...
- **Довірчі інтервали (Лекція 13).** Формула межі похибки довірчого
...
- **Перевірка гіпотез (Лекція 14).** Статистика критерію, з якої
...
механізм, який Лекції 13 і 14 просто називають іншими словами
(«похибка», «критерій», «p-value») і застосовують до конкретних задач.

```mermaid
flowchart LR
    A["Випадкова величина X<br/>E[X], Var(X) — фіксовані (Лекція 10)"] --> B["Вибірка x₁, x₂, ..., xₙ"]
    B --> C["x̄ₙ, s²"]
    C -->|"ЗВЧ: n → ∞ ⇒ x̄ₙ → E[X]"| A
    C -->|"ЦГТ: x̄ₙ ~ N(μ, σ/√n)"| D["Довірчі інтервали (Лекція 13)<br/>Перевірка гіпотез (Лекція 14)"]
```

Це та сама діаграма, з якою Лекція 10 закінчилась — тільки тепер
...
оцінку дисперсії — саме тому Лекція 7 і ця лекція наполягають на
...
- Точкова оцінка — правило, що обчислює наближення параметра з
  ...
  довільним вибором.
- Довірчі інтервали (Лекція 13) і перевірка гіпотез (Лекція 14) —
...
Моклячук М.П. *Лекції з теорії ймовірностей та математичної статистики.* — Київ: Київський національний університет імені Тараса Шевченка, 2020 ([відкритий доступ](https://mechmat.knu.ua/wp-content/uploads/2020/05/teor-imovi-mmp.pdf)). Розділ II: локальна (Муавра–Лапласа) та інтегральна граничні теореми — основа ЦГТ; теорема Пуассона як приклад закону великих чисел у схемі Бернуллі.

Веригіна І.В., Островська О.В., Проскурін Д.П. *Теорія ймовірностей та математична статистика: Збірник задач.* — Київ: КПІ ім. Ігоря Сікорського, 2019 ([відкритий доступ](https://ela.kpi.ua/items/d3fb4fee-184f-49a5-8eaf-523e2988e6f3)). Збірник задач на застосування граничних теорем і законів великих чисел для самостійного розв'язання.
