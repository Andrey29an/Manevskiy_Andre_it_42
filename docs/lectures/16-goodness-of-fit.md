...
*Лекція 14 ввела загальну логіку перевірки статистичних гіпотез (H0, H1,
p-value, рівень значущості α), а Лекція 15 застосувала цю логіку до
...
абсолютно ті самі, що в Лекціях 14–15. Насамкінець лекція повертається
до числових даних з іншого боку: як формально перевірити, чи вибірка
узгоджується з нормальним розподілом, — те, що в Лекції 11 робилося
...
довільний вибір, а пряме продовження логіки дисперсії з Лекції 7.
...
одразу повертає p-value — точно за тією логікою з Лекції 14: мале
...
## Хі-квадрат критерій незалежності: пряма відповідь на обіцянку з Лекції 8

Лекція 8 ввела `pd.crosstab()` — функцію, що рахує частоту кожної
...
Лекції 10: `P(A ∩ B) = P(A) · P(B)`). Тест рахує ту саму статистику χ², що
...
із Лекції 10. Якщо доля клієнтів, що платять карткою, серед **усіх**
...
**Прямий зв'язок із Лекцією 10.** Там був сформульований принцип: "незалежність
...
саме тут. Лекція 10 наводила приклад "дощ і хмарність" як залежні події,
...
Лекція 12). Коли `E` мале, дискретність можливих значень `O` (ціле
...
Лекція 11 у "Типових помилках" попереджала: перш ніж підбирати параметри
...
Лекції 15 (t-критерій) і багато інших статистичних методів курсу
...
логікою Лекції 14, а не відтворювати формулу вручну.
...
нормального розподілу (та сама квантильна функція з Лекції 11): якщо
...
(Лекція 11) + формальний тест", тільки тепер обидва інструменти точні:
графік показує форму відхилення, тест дає число, на основі якого можна
ухвалити рішення без суб'єктивної оцінки "на око".

**Де корисно.** Перед t-критерієм для малої вибірки (Лекція 15) —
...
n (Лекція 13) — та ж перевірка. У контролі якості — переконатись, що
вимірювана характеристика деталі (діаметр, вага) справді розподілена
нормально, перш ніж застосовувати правило 68-95-99.7 (Лекція 11) для
...
відхилити".** Це загальна пастка перевірки гіпотез з Лекції 14, яка
...
- Хі-квадрат критерій узгодженості перевіряє, чи розподіл однієї
  ...
  статистика `Σ (O-E)²/E` — та ж логіка, що дисперсія з Лекції 7:
...
- Хі-квадрат критерій незалежності перевіряє H0 "дві категоріальні
  ...
  (Лекція 8); очікувані частоти клітинок обчислюються за формулою
  `рядок × стовпець / загальна сума` — прямим наслідком визначення
  незалежності подій `P(A∩B) = P(A)·P(B)` з Лекції 10.
...
- Критерій Шапіро-Вілка (`scipy.stats.shapiro`) формально перевіряє H0
  ...
  Лекція 11 радила робити "на око" за гістограмою.
...
Каніовська І.Ю., Стусь О.В. *Математична статистика: Збірник задач.* — Київ: КПІ ім. Ігоря Сікорського, 2019 ([відкритий доступ](https://ela.kpi.ua/bitstream/123456789/27540/1/Matemat_statystyka.pdf)). Гл.4 «Критерій узгодженості Пірсона (χ²)»: перевірка гіпотези про вигляд розподілу, застосування в схемі Бернуллі, критерій незалежності — профільний розділ.

Єременко В.С., Куц Ю.В., Мокійчук В.М., Самойліченко О.В. *Статистичний аналіз даних вимірювань.* — Київ: Національний авіаційний університет (НАУ), 2013 ([відкритий доступ](https://ela.kpi.ua/bitstream/123456789/44902/1/statistical_analysis.pdf)). Гл.3 «Методи визначення закону розподілу даних»: ідентифікація розподілу за згрупованими/незгрупованими даними, критерії згоди.
