...
*Лекція 19 завершила Модуль 3 класифікацією математичних моделей і серед
...
Лекція 11 розглядала розподіли (рівномірний, нормальний, експоненційний
...
Це той самий словник параметрів, що й у Лекції 11 (`scale` для
...
- **Навчальні датасети з відомою правильною відповіддю** — саме цей
  ...
  лінійної залежності, і перевіряється, чи регресія (Лекції 17–18)
...
пряме застосування закону великих чисел (Лекція 12): якщо цільова
...
"вибіркового середнього як оцінки E[X]" (як у Лекції 12), а до
...
розподілу, Лекція 11, лише застосована до двовимірної фігури, а не до
...
середнє — це вибіркове середнє за визначенням (Лекція 12) наближається
до `E[g(X)]`.
...
для одержання `n_repeats` вибіркових середніх у Лекції 12 — генерується
...
Це точно та сама конструкція, що вибіркове середнє x̄ₙ у Лекції 12 — і
...
"вибіркового розподілу середнього", що й повторні вибірки в Лекції 12
...
стандартної похибки `σ/√n` із Лекції 12, застосована буквально:
...
обговорювався в Лекції 12 для x̄ — тут він застосований до оцінок
...
Пряме перенесення помилки з Лекції 12: якщо симуляція з `n=1000` дала
...
статистика", що вже зустрічався в Лекції 16 для критерію хі-квадрат.
...
- Метод Монте-Карло — оцінка детермінованої величини (площі, інтеграла,
  ...
  застосування закону великих чисел (Лекція 12) до довільної величини,
  переписаної у формі математичного сподівання.
- Похибка оцінки Монте-Карло убуває як `1/√n` (та сама стандартна
  похибка з Лекції 12): учетверо більше випробувань — лише вдвічі
...
Байда Є.І. *Метод Монте-Карло: навчально-методичний посібник.* — Харків: Національний технічний університет «Харківський політехнічний інститут», 2024 ([відкритий доступ](https://repository.kpi.kharkov.ua/items/cd2f2cfd-4e6a-4733-ad87-d9f39fe8f19e)). Профільний навчально-методичний посібник саме з методу Монте-Карло: типи розподілів, алгоритми й приклади коду для моделювання випадкових процесів.

Бідюк П.І., Романенко В.Д., Тимощук О.Л. *Аналіз часових рядів: навчальний посібник.* — Київ: НТУУ «КПІ», 2010 ([відкритий доступ](https://ela.kpi.ua/items/256da52e-13da-4d07-8f25-9d9505496a2c)). Згадка та застосування методу Монте-Карло для ланцюгів Маркова (MCMC) як методу оцінювання параметрів у контексті аналізу часових рядів.
